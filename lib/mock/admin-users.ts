...
const firstNames = [
  ...
  "William", "Amanda", "John", "Melissa", "Christopher", "Nicole", "Daniel", "Stephanie",
  "Matthew", "Jennifer", "Anthony", "Elizabeth", "Mark", "Rachel", "Steven", "Lauren",
  "Andrew", "Samantha", "Joshua", "Michelle", "Ryan", "Heather", "Kevin", "Kimberly",
  "Brian", "Christina", "Justin", "Amber", "Brandon", "Brittany", "Jason", "Angela",
  "Eric", "Megan", "Tyler", "Tiffany", "Aaron", "Natalie", "Adam", "Rebecca",
  "Nathan", "Danielle", "Zachary", "Kayla", "Patrick", "Victoria", "Sean", "Katherine"
]
...
function generateMockUsers(): AdminUser[] {
  ...
  for (const workspace of workspaces) {
    ...
    for (let i = 0; i < additionalMembers; i++) {
      ...
      const status: UserStatus = statusRoll < 0.7 ? "active" : statusRoll < 0.9 ? "pending" : "inactive"

      // Images generated for this user
      const userImages = status === "active"
      ...
      // Last active
      const lastActiveDaysAgo = status === "active" ? Math.floor(random() * 7) : Math.floor(random() * 60) + 30
...
export type SortableUserColumn = "name" | "email" | "role" | "status" | "imagesGenerated" | "lastActiveAt" | "joinedAt"

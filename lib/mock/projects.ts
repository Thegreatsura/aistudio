...
function generateMockProjects(count: number): Project[] {
  ...
  for (let i = 0; i < count; i++) {
    ...
    for (let j = 0; j < imageCount; j++) {
      const imageStatus = j < completedCount ? "completed" : status === "processing" && j === completedCount ? "processing" : "pending"
      images.push({
        id: `img_${i}_${j}`,
        originalUrl: `https://picsum.photos/seed/${i * 10 + j}/800/600`,
        enhancedUrl: imageStatus === "completed" ? `https://picsum.photos/seed/${i * 10 + j + 100}/800/600` : undefined,
...
export const ALL_PROJECT_STATUSES: ProjectStatus[] = ["pending", "processing", "completed", "failed"]

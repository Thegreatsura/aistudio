...
function generateMockWorkspaces(count: number): AdminWorkspace[] {
  ...
  for (let i = 0; i < count; i++) {
    ...
    const status: WorkspaceStatus = statusRoll < 0.8 ? "active" : statusRoll < 0.9 ? "trial" : "suspended"
...
export type SortableWorkspaceColumn = "name" | "memberCount" | "imagesGenerated" | "totalSpend" | "createdAt" | "lastActivityAt"
...
function filterWorkspaces(workspaces: AdminWorkspace[], filters: WorkspaceFilters): AdminWorkspace[] {
...
function sortWorkspaces(workspaces: AdminWorkspace[], sort?: [SortableWorkspaceColumn, SortDirection]): AdminWorkspace[] {
